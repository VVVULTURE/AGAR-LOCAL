# ── Agar.io Clone — Koyeb Docker Deployment ───────────────────────
...
RUN addgroup -S agario && adduser -S agario -G agario

WORKDIR /app

# Copy package files first for better layer caching
COPY --chown=agario:agario package.json package-lock.json ./

# Install only production dependencies, no native compilation
RUN npm install --omit=dev --ignore-scripts

# Copy the rest of the project
COPY --chown=agario:agario . .
