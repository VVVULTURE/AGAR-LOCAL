...
// Serve index.html dynamically so we can inject the real server URL.
// This makes socket.io connect directly to the game server even when the
// page is opened through a proxy (e.g. Red Portal) that re-serves it as
// a blob: URL — the injected SOCKET_URL always points back here.
app.get('/', (req, res) => {
    const proto = req.headers['x-forwarded-proto'] || (req.secure ? 'https' : 'http');
    const host  = req.headers['x-forwarded-host']  || req.headers.host;
    const socketUrl = proto + '://' + host;
    let html = fs.readFileSync(path.join(clientDir, 'index.html'), 'utf8');
    html = html.replace('</head>', '<script>window.SOCKET_URL="' + socketUrl + '";</script>\n</head>');
...
// Health check — load-balancers and Koyeb/Render poll this to confirm the container is alive
...
const serverport = parseInt(process.env.PORT || config.port || 3000, 10);
const ipaddress  = '0.0.0.0';

http.listen(serverport, ipaddress, () => {
    console.log('');
    console.log('  ╔══════════════════════════════════════════╗');
    console.log('  ║         Agar.io Clone — Local Server     ║');
    console.log('  ╚══════════════════════════════════════════╝');
    console.log('');
    console.log(`  🌐  Open:    http://localhost:${serverport}`);
    console.log(`  🎮  Share:   connect friends to the same URL`);
    console.log('');
...
http.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
        console.error(`\n  ✗  Port ${serverport} is already in use. Try: PORT=3001 node server.js\n`);
    } else {
